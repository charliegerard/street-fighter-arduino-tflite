...
class Game {
  ...
  moveFigther = (command) => {
    if (command.left) {
      positionX -= command.left;
    } else if (command.right) {
      positionX += command.right;
    }
  };
  ...
  animate = () => {
    ...
    if (this.spritePosition !== 4) {
      this.drawFrame(
        this.loop[currentLoopIndex],
        this.spritePosition,
        positionX,
        canvas.height - (scaledHeight + 30)
      );
    }

    if (this.spritePosition === 0) {
      // hadoken add sprite for fireball
      let fireballCycleLoop = [0, 1];

      this.drawFrame(
        fireballCycleLoop[currentLoopIndex],
        4,
        (fireballPosition += 35),
        canvas.height - (scaledHeight + 30)
      );
